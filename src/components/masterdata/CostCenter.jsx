...
import { FaPlus, FaTimes, FaTrash, FaSitemap, FaSearch, FaChevronLeft, FaChevronRight } from "react-icons/fa";
...
const initialForm = { cost_center_code: "", cost_center_name: "" };
const fields = [
...
const searchKey = "cost_center_code";
...
export default function CostCenter() {
    ...
    // DELETE — mode seleksi batch (logic sama seperti Employee / GL Account / Advance)
    ...
    const loadData = async (term) => {
        // hubungkan ke API Cost Center di sini
    };
    ...
    // ===== BATCH DELETE (logic sama seperti Employee / GL Account / Advance) =====
    ...
    return (
        <div style={{ animation: "slideDown 0.3s ease" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: "16px", paddingLeft: "10px", paddingRight: "10px", flexWrap: "wrap", gap: "12px" }}>
                <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-gray-500">Kode Cost Center</label>
                    <AutocompleteInput
                        value={searchTerm}
                        onChange={setSearchTerm}
                        onSelect={setSearchTerm}
                        suggestions={suggestions}
                        placeholder="Cari Kode Cost Center..."
                    />
                </div>

                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    {/* Button Tambah Cost Center — disembunyikan saat mode hapus aktif */}
                    {!deleteMode && (
                        <button
                            type="button"
            ...
            {modalOpen && (
                <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.45)", backdropFilter: "blur(4px)", padding: "16px" }}>
                    <div style={{ background: "#fff", borderRadius: "20px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", width: "100%", maxWidth: "440px", maxHeight: "90vh", overflowY: "auto", animation: "slideDown 0.25s ease" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px 16px", borderBottom: "1px solid #f1f5f9" }}>
                            <h3 style={{ margin: 0, fontSize: "16px", fontWeight: 700, color: "#59616F" }}>Tambah Cost Center</h3>
                            <button
                                type="button"
                                onClick={handleClose}
                        ...
                        <form onSubmit={handleSubmit} style={{ padding: "20px 24px 24px", display: "flex", flexDirection: "column", gap: "16px" }}>
                            {fields.map((field) => (
                                <div key={field.name}>
                                    ...
                                    <input
                                        ...
                                        style={{ width: "100%", border: "1.5px solid #e5e7eb", borderRadius: "10px", padding: "9px 12px", fontSize: "13px", outline: "none", boxSizing: "border-box", transition: "border-color 0.2s, box-shadow 0.2s" }}
                                        onFocus={(e) => {
                            ...
                            <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "4px", paddingTop: "16px", borderTop: "1px solid #f1f5f9" }}>
                                ...
                                <button
                                    type="submit"
                                    style={{ background: "linear-gradient(135deg, #363D48, #59616F)", border: "none", borderRadius: "10px", padding: "8px 20px", fontSize: "13px", color: "#fff", cursor: "pointer", fontWeight: 600, boxShadow: "0 4px 12px #59616F55" }}
                                >
                                    Simpan
                                </button>
                            </div>
